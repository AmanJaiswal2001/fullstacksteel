...
const ColdSheetCard = ({type,data=[]}) => {
    ...
    return (
        ...
        {filteredCards.slice(currentIndex, currentIndex + visibleCards).map((card, index) => (
            <Link key={card._id || index} to={`/coldproductsheet/${card._id || index}`} className='w-full flex justify-center gap-0 '>
         ...
         {filteredCards.length>visibleCards&&(
          ...
          <div className='mt-20 cursor-pointer flex justify-center mb-5 bg-[#12396d] border-[#2241a6] rounded-lg items-center m-auto w-[90%]  sm:w-[30%]'>
<Button
